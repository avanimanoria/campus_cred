...
const db = {
    ...
    // Executes a query and expects 0 or more results (alias)
    manyOrNone: (text, params) => {
        return dbInstance.manyOrNone(text, params);
    },
    ...
    // Executes a query and returns a result object (rowCount etc.)
    result: (text, params) => {
        return dbInstance.result(text, params);
    },
    // Run a transaction. Accepts a callback that receives the transaction root.
    tx: (txCb) => {
        return dbInstance.tx(txCb);
    },
};
