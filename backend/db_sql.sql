...
CREATE TABLE events (
  ...
  venue VARCHAR(255),
  faculty_id INT REFERENCES faculty_details(faculty_id),
...
-- External certificates uploaded by students for events
CREATE TABLE IF NOT EXISTS external_certificates (
  id SERIAL PRIMARY KEY,
  student_usn VARCHAR(50) NOT NULL REFERENCES students(student_usn) ON DELETE CASCADE,
...
-- Events sample
INSERT INTO events (title, description, category, start_at, end_at, venue, faculty_id, dept_id, status)
